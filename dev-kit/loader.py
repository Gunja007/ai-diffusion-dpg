...
_KIT_ROOT = Path(__file__).parent
...
def _deep_merge(base: dict[str, Any], override: dict[str, Any]) -> dict[str, Any]:
    ...
    - Lists: override replaces (no element-level merging).
    """
    result = base.copy()
...
def _load_and_merge(domain: str, service: str) -> dict[str, Any]:
    """
    Load and merge DPG defaults + domain config for a given service.
    Returns the merged raw dict (not yet validated).
    """
...
def validate_all(domain: str) -> dict[str, bool]:
    ...
    Returns {service_name: True/False} — True means valid.
...
def build_all(domain: str, output_dir: Path) -> None:
    """
    Merge DPG defaults + domain values for all 7 services and write
    merged YAML files to output_dir/{service}.yaml.
    These merged files are ready to be volume-mounted into containers.
    """
