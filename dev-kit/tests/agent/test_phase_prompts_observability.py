"""Tests for dev_kit.agent.phase_prompts.observability."""
...
def _fake_field(path: str, description: str = "A field"):
    ...
    rule = FieldRule(category="chat", phase="observability", description=description)
    return (path, rule)


from dev_kit.agent.phase_prompts.observability import build
...
def test_build_contains_phase_header():
    result = build([], "", "", _intake())
    assert "# Phase: Observability" in result
...
def test_build_renders_pending_fields():
    fields = [
        _fake_field("observability_layer.outcomes.lifecycle", "Outcome lifecycle states"),
        _fake_field("observability_layer.quality.signals", "Quality signals to track"),
    ]
    result = build(fields, "", "", _intake())
    assert "observability_layer.outcomes.lifecycle" in result
    assert "Outcome lifecycle states" in result
    assert "observability_layer.quality.signals" in result
    assert "Quality signals to track" in result
